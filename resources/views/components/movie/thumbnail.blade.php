<div
    class="uk-inline uk-dark uk-visible-toggle uk-animation-toggle">
    @can('viewAny', \App\Models\Movie::class)
        <img loading="lazy"
             src="/images/thumbnails/movie{{Illuminate\Support\Str::padLeft($movie->id,3,'0')}}.jpg"
             width="200"
        ...
        @if($overlay)
            <div
                ...
                <div class="uk-position-top uk-height-1-1">
                    ...
                    <a href="{{ \Illuminate\Support\Facades\Gate::check('view', $movie) ? route('login') : (\Illuminate\Support\Facades\Gate::check('watch', $movie) ? route('movie.show', ['movie'=>$movie->hashid()]) : (route('profile') . "#choose-plan") ) }}"
            ...
            <div
                class="uk-overlay uk-hidden-notouch uk-position-cover" {!! (($overlay ?? true) ? 'uk-toggle="target: #movie-modal-'.$movie->hashid().'"' :'') !!}></div>
                                ...
                                @else
                                    <a href="{{route('profile') . '#choose-plan'}}" class="uk-button uk-button-primary"
                                       type="button">Choose
                                        a plan to watch
                                        <span
                                            class="uk-margin-small-right" uk-icon="lock"></span></a>
                                @endcan
                            @else
